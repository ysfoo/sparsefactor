...
where $l_{ik}$ is the regulation strength of factor $k$ on gene $i$, $f_{kj}$ is the activation weight of factor $k$ for individual $j$, and \(e_{ij}\) is a noise term. To achieve sparsity, $l_{ik}$ is set to zero if gene $i$ is not regulated by factor $k$. Further detail can be found in `docs/assets/report.pdf`.

Following a Bayesian approach, the posterior distributions of the parameters of interest (e.g. $l_{ik}$ and $f_{kj}$) are intractable to calculate directly. Two techniques are implemented:
